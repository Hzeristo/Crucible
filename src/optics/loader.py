"""从 ``lenses_dir`` 加载 Lens 配置；缺失时生成内置三透镜并落盘。"""
...
from .schema import LensConfig

logger = logging.getLogger(__name__)

_DEFAULT_FILE_STEM = "default_lenses"

# 排版军规：反代码块包裹 + 长叙事分段（见各 _JSON_*）
...
def _builtin_lens_configs() -> list[LensConfig]:
    """Sprint 4 默认三透镜：硬核 persona + 严格 JSON 受体。"""
    return [
        LensConfig(
            id="math_arch",
...
def _builtin_survey_lens_configs() -> list[LensConfig]:
    """综述专用三透镜：分类拓扑、共识瓶颈、结构空白（拒绝文献表堆砌）。"""
    return [
        LensConfig(
            id="survey_taxonomy",
...
def load_survey_lens_configs(settings: Settings | None = None) -> list[LensConfig]:
    """
    读取 ``{lenses_dir}/survey_lenses.yaml``（或 ``.yml`` / ``.json``）。
    缺失或解析失败时返回内置综述三透镜并尝试写入 ``survey_lenses.yaml``。
    """
    cfg = settings or load_config()
    base: Path = cfg.lenses_dir
...
def load_lens_configs(settings: Settings | None = None) -> list[LensConfig]:
    """
    读取 ``{lenses_dir}/default_lenses.yaml``，否则 ``default_lenses.json``。
    目录或文件不存在、或解析失败时，返回内置三透镜并写入 ``default_lenses.yaml``。
    """
    cfg = settings or load_config()
    base: Path = cfg.lenses_dir
